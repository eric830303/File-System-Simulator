...
int FileSystem::regist( string username, string password )
{
    ...
    //No repetition
    FILE *adduser = fopen("./setting/user.txt","a") ;
    if( !adduser )
    {
        printf( RED"[Error] " RESET"Cannot open ./user.txt\n");
    }
    fprintf( adduser,"%s\t%s\n", username.c_str(), password.c_str());
    MyDir *newUserDir = new MyDir( NULL, NULL, NULL, NULL, 0 );
    newUserDir->name = username ;
    
    /*Rebuild the Linked List*/
    newUserDir->preDir  = this->root ;
    newUserDir->nextDir = this->root->dirPtr;
    this->root->dirPtr = newUserDir ;
...
{
    ifstream token ;
    token.open( "./setting/user.txt", ifstream::in ) ;
    if( !token )
    {
        printf("Error!\n");
    }
...
//* Editor: Tien-Hung Tseng                                                              *
//****************************************************************************************
void  FileSystem::BuildFile( string FileName, int filecount, MyDir *currentDir, int id )
{
    ...
    if( currentDir->filePtr == NULL )
    {
        ...
        this->getFileMap().insert( pair<int,MyFile*>(id,newFile) );
    }
    else
    {
...
void  FileSystem::BuildDirFile( ifstream & token, vector<string> strspl, MyDir *currentDir )
{
    ...
    int size = stoi( strspl[2] );//size of dir, id of file
    int count = 0 ;
    //--- File --------------------------------
    if( strspl[1] == "f" )
    {
        count = stoi( strspl[3] ) ;
        BuildFile( strspl[0], count,currentDir, size );
        return ;
    }
    //--- Directoy ----------------------------
    else if( strspl[1] == "d" )
...
{
    ifstream token ;
    token.open( "./setting/tree.txt", ifstream::in ) ;
    assert( token ) ;
    ...
    while( token >> line )
    {
        ...
        assert( strsplit.size() == 3 );//user home dir
...
//* Editor: Tien-Hung Tseng                                                              *
//****************************************************************************************
void FileSystem::dumpDirContent( FILE* f, FILE*f2, MyDir* currentDir, int layer )
{
    //---- Check ----------------------------------------------------
    ...
    while( itrDir )
    {
        this->dumpDirContent( f, f2, itrDir, layer+1 ) ;
        itrDir = itrDir->nextDir ;
    }
    ...
    while( itrFile )
    {
        ...
        fprintf( f , "%s%s/f/%d/%d\n", tabcount2.c_str(), itrFile->name.c_str(), itrFile->fid, itrFile->count );
        fprintf( f2, "%d\t%s\n", itrFile->fid, itrFile->content.c_str() );
...
void FileSystem::dumpDirTree()
{
    ...
    FILE *dirtree = fopen("./setting/tree.txt","w") ;
    FILE *dmpfile = fopen("./setting/file.txt","w") ;
    if( !dirtree )
    {
        assert( dirtree );
        printf( RED"[ERROR]" RESET"Cannot open tree.txt\n" );
    }
    dumpDirContent( dirtree, dmpfile, this->root, 0 ) ;
}
//****************************************************************************************
//* Fun Name: readFileContent()                                                          *
//* Work: read the file content before execution                                         *
//* Where to be called: run()                                                            *
//* Editor: Tien-Hung Tseng                                                              *
//****************************************************************************************
void FileSystem::readFileContent()
{
    ifstream  F    ;
    char      line[1024] ;

    F.open( "./setting/file.txt",ios::in );
    if( !F )
    {
        printf( RED"[ERROR]" RESET"Cannot open file.txt\n" );
        assert( F );
    }
    while( F.getline( line, sizeof(line), '\n' ) )
    {
        char *strspl = strtok( line, "\t" ) ;
        auto itr = this->getFileMap().find( atoi(strspl) );
        if( itr != this->getFileMap().end() )
        {
            strspl = strtok( NULL, "\t" );
            
            itr->second->content = strspl ;
            itr->second->size = (int)(itr->second->content.length()) ;
            
            MyDir *h = itr->second->preDir;
            
            /*Resize the size of upper layer direcotory*/
            while( h != NULL )
            {
                h->size = h->size + itr->second->size;
                h = h->preDir;
            }
        }
    }
